...
from flask import Flask, request  # request object encapsulates the HTTP request sent by the client
...
REQUEST_LATENCY = Histogram(
    name="http_request_latency_seconds", documentation="Request latency", labelnames=["app_name", "endpoint"]
)
...
def stop_timer(response: Response) -> Response:
    ...
    REQUEST_LATENCY.labels(app_name=APP_NAME, endpoint=request.path).observe(request_latency)
    return response
...
def record_request_data(response: Response) -> Response:
    ...
    REQUEST_COUNT.labels(
        app_name=APP_NAME, method=request.method, endpoint=request.path, http_status=response.status_code
    ).inc()
    return response
